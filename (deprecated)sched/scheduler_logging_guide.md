# vLLM Scheduler Logging Implementation Guide
...
| 파일 | 변경 내용 |
|------|----------|
| `vllm/v1/core/sched/scheduler_logger.py` | **[신규 생성]** 스케줄러 로깅 모듈. `SchedulerLogger` 클래스와 관련 데이터 클래스(`RequestMetrics`, `IterationMetrics`, `PerIterationRequestState`, `SchedulerEvent`) 구현. 환경변수 기반 설정, 버퍼링된 CSV 출력, 비동기 flush 지원. |
| `vllm/v1/core/sched/scheduler.py` | **[수정]** 스케줄러 로깅 통합. `SchedulerLogger` 초기화, request 도착/스케줄/preemption/완료 이벤트 기록, iteration 시작/종료 로깅, token 생성 및 TTFT 추적, shutdown 시 로거 정리. |
| `vllm/v1/core/sched/request_queue.py` | **[수정]** `RequestQueue` 추상 클래스에 `get_all_requests()` 메서드 추가. `FCFSRequestQueue`와 `PriorityRequestQueue`에 구현. |

### 주요 구현 기능

1. **Request-level 메트릭**
   - arrival_time, first_scheduled_time, completion_time 추적
   - waiting_time, execution_time 누적 계산
   - TTFT (Time To First Token), TPOT 계산
   - preemption_count, kv_cache_blocks 추적
   - state transition 히스토리 기록

2. **Iteration-level 메트릭**
   - iteration 시작/종료 시간, scheduling overhead 측정
   - batch_size, prefill_tokens, decode_tokens 기록
   - running/waiting/preempted request ID 목록
   - KV cache 사용량 (used/total blocks)
   - scheduler action 기록 (admit_new, preempt, resume, continue)

3. **Event 로깅**
   - request_arrived, request_scheduled, request_preempted
   - request_completed, first_token_generated
   - kv_cache_allocated, kv_cache_freed

4. **출력 파일** (CSV 형식)
   - `per_request.csv`: Request별 요약 통계
   - `per_iteration.csv`: Iteration별 시스템 상태
   - `per_iteration_requests.csv`: Iteration별 각 request 상태
   - `events.csv`: 모든 이벤트 로그
...
# 로그 확인
ls scheduler_logs/
# per_request.csv, per_iteration.csv, per_iteration_requests.csv, events.csv
```

### 성능 고려사항

- 버퍼링 (기본 100개 항목)으로 I/O 오버헤드 최소화
- Thread lock으로 동시성 안전 보장
- atexit 핸들러로 프로그램 종료 시 자동 flush
- 환경변수로 로깅 비활성화 시 오버헤드 제로
...
- 필요시 절대 timestamp도 함께 기록

### 6.2 성능 고려사항
- 로깅이 스케줄러 성능에 영향을 최소화해야 함
- 가능하면 비동기 로깅 구현
- CSV 버퍼링 및 주기적 flush (예: 100 iterations마다)
