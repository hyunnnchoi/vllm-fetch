...
from vllm.v1.core.sched.scheduler_logger import (
    SchedulerLogger,
    WaitingReason,
...
class Scheduler(SchedulerInterface):
    def __init__(
        ...
        # Initialize scheduler logger for detailed metrics collection
        self.scheduler_logger: SchedulerLogger | None = create_scheduler_logger()
    ...
    def schedule(self) -> SchedulerOutput:
        ...
        # Scheduler logging: begin iteration
        scheduler_actions: list[str] = []
        if self.scheduler_logger is not None:
        ...
        while req_index < len(self.running) and token_budget > 0:
            ...
            with record_function_or_nullcontext("schedule: allocate_slots"):
                while True:
                    ...
                    # Scheduler logging: record preemption
                    if self.scheduler_logger is not None:
                        self.scheduler_logger.on_request_preempted(preempted_req)
                        scheduler_actions.append("preempt")
        ...
        if not preempted_reqs:
            while self.waiting and token_budget > 0:
                ...
                if request.status == RequestStatus.WAITING:
                    ...
                    # Scheduler logging: record new request scheduled
                    if self.scheduler_logger is not None:
                        self.scheduler_logger.on_request_scheduled(
                            request, is_first_schedule=True
                        )
                        scheduler_actions.append("admit_new")
                elif request.status == RequestStatus.PREEMPTED:
                    scheduled_resumed_reqs.append(request)
                    # Scheduler logging: record resumed request scheduled
                    if self.scheduler_logger is not None:
                        self.scheduler_logger.on_request_scheduled(
                            request, is_first_schedule=False
                        )
                        scheduler_actions.append("resume")
                else:
        ...
        # Scheduler logging: end scheduling phase and record iteration
        if self.scheduler_logger is not None:
            self.scheduler_logger.end_scheduling_phase()

            # Get waiting request IDs
            waiting_request_ids = [req.request_id for req in self.waiting]

            # Calculate KV cache stats
            kv_cache_total = self.kv_cache_manager.block_pool.num_gpu_blocks - 1
            kv_cache_used = int(self.kv_cache_manager.usage * kv_cache_total)

            # Add continue action if we have running requests but no other actions
            if scheduled_running_reqs and not scheduler_actions:
                scheduler_actions.append("continue")

            self.scheduler_logger.end_iteration(
                running_requests=self.running,
                waiting_request_ids=waiting_request_ids,
                num_scheduled_tokens=num_scheduled_tokens,
                preempted_request_ids=[r.request_id for r in preempted_reqs],
                admitted_request_ids=[r.request_id for r in scheduled_new_reqs],
                completed_request_ids=list(self.finished_req_ids),
                kv_cache_used_blocks=kv_cache_used,
                kv_cache_total_blocks=kv_cache_total,
    ...
    def update_from_output(
        ...
        for req_id, num_tokens_scheduled in num_scheduled_tokens.items():
            ...
            if new_token_ids:
                # Track if this is the first token for TTFT
                was_first_token = request.num_output_tokens == 0

                new_token_ids, stopped = self._update_request_with_output(
                    request, new_token_ids
                )

                # Scheduler logging: record token generation
                if self.scheduler_logger is not None and new_token_ids:
                    for _ in new_token_ids:
                        self.scheduler_logger.on_token_generated(req_id)

                    # Record first token event
                    if was_first_token and new_token_ids:
                        self.scheduler_logger.on_first_token_generated(
                            req_id, self.scheduler_logger.iteration_id
                        )
            ...
            if stopped:
                # Scheduler logging: record request completion
                if self.scheduler_logger is not None:
                    self.scheduler_logger.on_request_completed(request)
    ...
    def add_request(self, request: Request) -> None:
        ...
        # Scheduler logging: record request arrival
        if self.scheduler_logger is not None:
            self.scheduler_logger.on_request_arrived(request)
